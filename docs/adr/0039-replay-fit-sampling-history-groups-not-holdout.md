...
**未抽選群 U / 抽選済み群 S を、測定時点の sampleable range を既存の `sampled_once_` フラグで二分して定義し、両群と実 PER バッチを学習条件から独立した共通条件（FP32・eval mode・固定 midpoint 分位点・現行と同じ target 式）で評価する**ことを決定する。母数と平均年齢は snapshot の 1 回走査で求め、per-entry の抽選回数カウンタや書込時刻キャッシュは持たない。測定は optimizer 更新と target 同期の前に行い、target 組立とサンプル別誤差は learner ごとの 2 関数（NoGrad の `BuildTarget`、純粋な `ComputeElementError`）へ抽出して**学習経路も同じ関数を呼ぶ**。IQN の target taus と Munchausen 用 current 出力は呼び出し側が渡すので、学習側は現位置で RNG を消費し、診断側は固定 midpoint を渡す。hard UQE × IQN の target 行動選択は、呼び出し側が生成した risk taus を policy の `SelectAction` へ注入する入口で固定 midpoint 化し、スコア計算は policy 実装を使う。ReplayBuffer 抽象には母数・年齢・U 抽出・S 抽出を 1 lock 区間で組み合わせて要求できる `ProbeSamplingHistory` を加え、常に両群を抽出する API にはしない。購読ヒント（ADR 0031）で購読された出力に必要な処理だけを、その出力の cadence で実行する。
