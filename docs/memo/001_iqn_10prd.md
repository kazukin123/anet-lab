...
- 関連: ADR 0018（IQN を bind `*` DAG 汎用化と最小 Head で表現する）、ADR 0019（IQN+UQE の Q 系値は同一 forward の risk-biased action score を使う）、CONTEXT.md「taus」「tau配置方式」
...
| D3 | policy 側 tau_rule デフォルト: train=random×32 / eval=fixed×32 / target=fixed×32 |
...
| D5 | UQE+IQN は全対応: 下限＝減衰後の実効 tau（spatial 時は per-env tensor）、`uqe_use_tail_mean=false` は通常全点固定、D19のfull query有効時は同値な1点で`Z_τ`を推定 |
...
| D15 | **taus 検証は統合案（D7 置換）**: ①NN 一般の汎用チェックとして構築時に input_specs×全 bind factorおよびdirect `net.body.output` mappingを突合し、どちらにも参照されない入力だけを LOG::warn（IQN 固有ではない）②IQNHead/IQNDuelingHead factory で dummy rank==3・Dueling B/K 一致を初期化時 fail-fast ③設定レベル taus-bind fail-fast は廃止、推移的到達性 BFS はやらない（NN 設定の整合は使う側責任） |
...
| D19 | IQN+UQE は任意の `full_distribution_query` を持つ。enabled時はrisk tausとfull `[0,1]` tausを連結して1回だけforwardし、risk側を`q_values`/`uqe_values`/`q_quantiles`、full側を`full_q_values`/`full_q_quantiles`へ分離する。point UQEのrisk側はα 1本とし、QValuePanelはfull分布を優先する。既定disabled。非IQN modeではenabled設定を休眠状態として無視し、IQN+非UQEでのenabledだけをfail-fastする |
| D20 | TauGeneratorは専用`tau_generator.hpp/.cpp`を新設せず、DQN系Policy/Learnerと同じ機能グループの`dqn_based_agent.hpp/.cpp`へ同居させる。AGENTS.mdの機能グループ単位のファイル方針に従い、statelessな`GenerateTaus`のAPI・意味論は変更しない |
...
- QValuePanel は通常`aux_data["q_quantiles"]`を読み、任意の`full_q_quantiles`があればそちらを優先する。
- 現用設定は`agent.txt`で既定契約を定義し、LunarLander/DropMergeがIQN profileとeval full queryを有効化する。Rainbowと過去artifactは旧QR契約を歴史記録として維持する。
...
train_policy.tau_rule.sample_mode = random      # random / fixed（train デフォルト: random）

eval_policy.tau_rule.num_taus = 32
eval_policy.tau_rule.sample_mode = fixed            # eval デフォルト: fixed（決定的評価）
eval_policy.full_distribution_query.enabled = false
eval_policy.full_distribution_query.tau_rule.num_taus = 32
eval_policy.full_distribution_query.tau_rule.sample_mode = fixed

target_policy.tau_rule.num_taus = 32
target_policy.tau_rule.sample_mode = fixed          # target デフォルト: fixed
...
- `ActionPolicyConfig` へ追加: `TauRuleConfig tau_rule;`、`FullDistributionQueryConfig full_distribution_query;`、`std::string quantile_mode = "none";`。full queryは既定disabled、fixed×32。quantile_mode は**設定ファイルから直接読まない**（Agent 直下の値を自動引継ぎ。個別設定キーは作らない）。
...
- `DefaultDQNAgentConfig` へ追加: `std::string quantile_mode = "qr";`、`struct QrConfig { int num_quantiles = 51; } qr;`。quantile mode、各policy tau rule/full query、`learner.iqn.*`を読む。evalの配置既定はfixedとする。
- **target policyのtau ruleとfull query既定の代入位置は `use_optimistic_target` 分岐の後・target系READ_CONFIGの直前**とする。train policyのコピー後もtargetはfixed×32・full query disabledへ戻し、targetへの明示設定だけを優先する。
...
- **検証（fail-fast、ANET_SYSTEM_ERROR、キー名・指定値・期待値を含める）**:
  ...
  - `tau_rule.num_taus <= 0`（policy 3・full query 3・learner 2）、`sample_mode` ∉ {random, fixed}
  - `quantile_mode==iqn`かつ`full_distribution_query.enabled=true`かつ`policy_type!=UQE`（非IQN modeでは休眠設定として無視）
...
- **整合検出（D15/D18-2、D7 の設定レベル fail-fast を置換。AGENTS.md「汎用機構と利用側の責任境界」に従う）**: フレームワークが検証するのは**局所的・決定的に判定できる契約**のみで、taus の推移的到達性やアルゴリズム上の妥当性（taus が最終出力へ意図どおり寄与すること）は **NN 設定者の責任**とする（BFS 等による到達性証明は行わない）。フレームワーク側の機構は次の 2 つ:
  - **汎用「未使用入力 WARN」（NN 一般機構、IQN 固有ではない）**: `NetworkBodyBuilder::Build` で、input_specs のキーのうち**全 branch の bind factor（平坦化後）にもdirect `net.body.output` mappingにも現れないもの**だけを LOG::warn で報告する（キー名・両方から参照されていない旨を含む。1 build 内で対象キーごと 1 回）。これは**直接参照の書き忘れを知らせる診断**であり、bindまたはdirect mappingされていることが到達性や意味的寄与を保証するわけではない。iqn で taus を bind し忘れ、direct mappingもしない場合はこの WARN の対象になる。
...
- D20に従い、専用ファイルは新設せず`core/anet-core/src/dqn_based_agent.hpp` / `.cpp`へDQN系の機能グループとして配置する（namespace `anet::rl::dqn`）。
- 責務: **tau 配置方式（random / fixed）だけ**を担当する。範囲・本数は引数で受ける。tail という policy 概念は持たせない。stateless（メンバ状態なし。free 関数群または static のみのユーティリティ）。
...
- `fixed`: 指定範囲をK個の等幅区間に分け、各区間の中心へτを固定配置する。式は `tau_i = tau_min + (i+0.5)/K·(tau_max−tau_min)`、i=0..K−1で、RNG を消費しない。**全バッチ共通範囲版**では全バッチ同一（(1,K) 生成 → (B,K) へ expand で可）。**per-env 下限 overload では `tau[b,i] = l[b] + (i+0.5)/K·(tau_max − l[b])` とバッチごとに異なる**（expand 不可。sample_mode × overload の 4 通り全てを定義する）。
...
- **UQE**（iqn 経路。既存の vectorized UQE＝QR 用 sort/mask 経路は QR 専用に残す）:
  ...
  - `uqe_use_tail_mean=false`: 通常はtausを全点その下限値に固定する。full query有効時はrisk側を下限値1本にし、`Z_τ`を同じ意味のまま計算する。
  - aux の `uqe_values` には mean 結果を格納（既存キー維持）。greedy_only の ε ゼロ化は既存踏襲（UQE の楽観基準は greedy_only でも維持、という既存セマンティクスも踏襲）。
  - full query有効時はrisk tausの後へfull `[0,1]` tausを連結して1回だけforwardし、`q_dist`を分割する。`q_values`/`uqe_values`/`q_quantiles`はrisk側、`full_q_values`/`full_q_quantiles`はfull側とする。Headが全tausから返す`q`は使わない。
  - actor_q_hintとQ系メトリクスはfull queryの有無にかかわらずrisk-biased action scoreを使う。full分布は可視化・将来診断用であり、追加forwardは行わない。
- **ThompsonSampling**（iqn 経路）: 非 spatial は範囲 [0,1] × tau_rule（sample_mode は設定に従う。通常 random）で生成 → mean(2) を action score。spatial は UQE と同じ per-env 下限挙動（既存の spatial 分岐位置を踏襲）。
  - **注記（D6）**: **num_taus=1 かつ sample_mode=random** で古典 Thompson（env ごとに 1 サンプル分位で選択）相当。`fixed×1` は常に τ=0.5（中央値 greedy）になりランダムサンプルにはならない。デフォルト 32 では E[Z] に近づき楽観化はマイルドになる。古典挙動が欲しい構成は `train_policy.tau_rule.num_taus = 1`（sample_mode は random のまま）を明示設定する。
...
- `UpdateFromSamples` は QRLearner::UpdateFromSamples をベースに以下の差分:
  ...
  4. NoGrad 内: `SelectTargetActions(next_obs)`（既存再利用）— target_policy が**自分の tau_rule**（D3: fixed×32）で taus を注入する。Learner はこの forward の taus に関与しない
...
- QValuePanel: `full_q_quantiles`があれば優先し、なければ`q_quantiles`、最後に`q_values`へfallbackする。risk/fullのscalar metric追加は行わない。
...
- TauGenerator — DQN系機能グループの`dqn_based_agent.hpp` / `.cpp`へ配置
- `core/anet-core/src/rainbow_agent.cpp` — 機械的追従
- `apps/runner/config/agent.txt` / `LunarLander.txt` / `nn.txt` — 設定書き換え・IQNプロファイル・コメント一覧追記
- `apps/runner/config/DropMerge.txt` — 2048次元backboneとV/A各1024次元streamを維持したIQNを現用化し、evalだけfull queryを有効化。`GridMaze.txt`はコメント例のみ更新
- `docs/design/130_neural_networks.jp.md` — bind 記法へ `*` 演算子・bind_concat_dim・未使用入力 WARN（構築時の input_specs×全bind factorおよびdirect `net.body.output` mappingの突合）を追記、CosineEmbedding を module type として反映
...
2. **TauGenerator**: random の range・shape・同 seed 再現、fixed の値位置（(i+0.5)/K の lerp）と RNG 非消費、per-env 下限 overload、不正 sample_mode エラー（CUDA 環境では device 生成も）
...
5. **ActionPolicy**: iqn で全 policy_type（EpsilonGreedy/Greedy/UQE/Thompson）の SelectAction が成功し、**入力 obs dict に taus が残らない**（非汚染）。UQE の実効 tau 下限、spatial per-env、tail_mean=false、Thompson の random 生成。full queryはpointのα1本＋full K、tailのrisk K＋full Kを単一forwardで処理し、risk scoreへfull側を混入させない
...
7. **fail-fast / WARN**: quantile_mode 不正値 / qr で num_quantiles<=1 / UQE・Thompson × none / iqn で quantile_huber_kappa<=0 / iqn×UQE・Thompson で uqe_tau_start・end が非有限または [0,1] 外。**未使用入力 WARN（D15）**: input_specs のキーをどの branch も bind factorとして参照せず、direct `net.body.output` mappingもしない構成で LOG::warn が出ること（IQN に限らない一般テストとして）。direct mappingされたinputではWARNしないこと。**1 build 内で同一キーの WARN が重複しない**こと。テストは「WARN が出る/出ない」の診断挙動のみを検証し、**意味的到達性の保証としては使わない**
8. **Config**: デフォルト値（qr / 51 / 32×3 policy / 64×2 learner / sample_mode 既定）、quantile_mode の 3 policy への自動引継ぎ、full queryの既定disabled・値検証・IQN+UQE限定、use_optimistic_target=trueでもtargetのtau ruleとfull query既定を復元し明示設定だけを反映すること
...
- **probe 経路**: `GetTensorDictFunction`（SweepHeatMapPanel の状態スイープ等）は obs だけを渡すため、IQN では branch 入力欠落エラーになる（無症状ではなく明示エラーで止まる）。恒久対応（eval tau_rule の fixed を wrapper で自動補完する案）は後続。
- **IQN+UQE の q 系値は risk-biased action score**: §7 の仕様注記のとおり、`uqe_use_tail_mean=true` は upper-tail mean、`false` は `Zτ` であり、いずれも全分布平均 `E[Z]` ではない。aux q_values / actor_q_hint（近似Actor初期優先度）/ 平均 Q 系メトリクスが QR-UQE と系統差を持つ（ADR 0010 の許容範囲として扱う）。
- **計算量**: IQN は fusion 以降と Head の中間テンソルが (B,K,·) になり、Kに比例してメモリ・演算が増える。full query有効時はtail UQEがrisk K＋full K、point UQEが1＋full Kとなるため、DropMergeではevalだけ有効化してprofileする。
...
- probe / TensorDictFunction 経路への taus 自動補完（fixed）
