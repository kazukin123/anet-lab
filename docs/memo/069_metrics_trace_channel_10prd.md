...
- **`metrics.trace.defs`** を新設。attach 済み trace 定義（dormant eval tag は除外）を tag 単位で `{tag: {step_axis, runner, scope, eval_name, eval_episodes, num_envs, event, target, keys:[...]}}`。書き口は `Log(tag, json)`（`type:"json"`、`json/metrics.trace.defs.json` ミラーも既存機構で出る）。空なら書かない。
- **追加合意（2026-09-05）**: scalar / trace の両定義に `scope`（`train` / `eval`）、`eval_name`、`eval_episodes`（1セッションの採用予定数）、`num_envs`（構築済み eval Env の lane 数）を持たせる。train scope の eval 情報は `null`。`runner` は F7 の step counter 所有者のまま維持する。eval 条件は各 metric 定義へ重複保存し、SHARED の lane 数をエピソード group 数へ変換しない。採用予定数は完了の証拠ではない。scalar には `clip`（未指定 `null`）も追加する。Factory が購読先と clip を保持し、RunManager が attach 済み定義へ eval の実条件を付与する。
- **追加情報の reader 契約**: `inspect_run tags` は追加項目を JSON / Markdown の両方に出し、master / cache の両経路で保持する。過去 Run の定義に無い追加項目は不明（`null`）とし、tag や config から補わない。定義不在時の既存 config 導出経路では購読先と clip を復元するが、実際の eval 条件は推測しない。trace ダンプ CLI は別件とする。
...
5. **定義レコード**: scalar は `metrics.scalar.defs` へ改名し、event token に `session_end` を追加する。trace は §4.7 の定義スキーマを `metrics.trace.defs` に記録する。attach 済み定義だけを記録し、dormant eval は除外、trace 定義が空ならそのレコードを出さない。ミラーは `json/metrics.scalar.defs.json` / `json/metrics.trace.defs.json` に同じ内容を出力する。追加合意の購読先・eval 条件・clip は、tag 非依存、採用予定数と lane 数が異なる場合、SHARED、train の null、旧定義の欠落、config 導出を検証する。
