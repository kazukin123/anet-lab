...
`docs/memo/013_sample_prefetch_10prd.md` の Sample+To プリフェッチは実装済み（実測 8.8h→7.3h）。ただし現状は **`Learner` 内に prefetch ロジックが埋め込まれ**（`SamplePrefetchState`）、`Push()` / `UpdatePriorities()` と in-flight prefetch の順序境界も Learner 実装に漏れている。これを `ReplayBuffer` decorator に閉じ、replay ordering を固定する。

現在の追加実験では、`learner.use_rb_prefetch=true` の意味を **Sample+To prefetch + armed 後 Push の write-behind insert** へ拡張する。`Push` は mutation なので public `ReplayBuffer` IF は増やさず、prefetch armed 後だけ runner 側で stable 化済みの `BatchExperience` を shallow lifetime snapshot として同じ FIFO worker へ遅延投入する。

本リファクタは：
1. prefetch を **共通 decorator `PrefetchingReplayBuffer`（`ReplayBuffer` を wrap し、`ReplayBuffer` インターフェースを継承して提供する）に分離**。`Learner` を薄くする。
2. **replay ordering を固定**：`PrefetchingReplayBuffer::Push()` は prefetch armed 後だけ shallow lifetime snapshot 付きで同じ FIFO worker へ遅延投入し、`UpdatePriorities()` は次 prefetch future を `wait()` してから inner mutation へ進む。**公開 `ReplayBuffer` インターフェースは不変**。
3. overlap（`Sample+To` prefetch）と速度（8.8→7.3h 近傍）は維持。

責務分担：**replay ordering＝PrefetchingReplayBuffer の 1-deep prefetch + armed 後 Push write-behind + UpdatePriorities 前 wait / データ整合性＝ReplayBuffer の storage・metadata lock / overlap＝Sample+To prefetch / public RB IF＝不変**。
...
- **N-deep 化（AsyncDataLoader）は対象外**。今回は 1-deep prefetch と armed 後 Push write-behind に限定する（`docs/memo/999_async_loader_ndeep.md` 参照）。
...
### 1.4 config
`learner.use_rb_prefetch`（`agent.hpp` LearnerConfig、default false）。`default_dqn_agent.hpp` で `ANET_READ_CONFIG` 済み。**config key は変更しない**。現在の `true` は Sample+To prefetch と armed 後 Push write-behind の両方を有効化する。
`RainbowAgentConfig` は現在 `learner.use_rb_prefetch = false` に固定しており、今回も現状維持する。

### 1.5 replay ordering の機構（核心）
`PrefetchingReplayBuffer` は in-flight prefetch を常に 1 本に限定する。cold/warmup の `Push(caller)` は同期 inner forward のままにし、prefetch armed 後の `Push(caller)` は runner 側で stable 化済みの `BatchExperience` を shallow copy して同じ FIFO worker へ `inner_->Push(snapshot)` を enqueue する。`Sample()` が次 `Fetch()` を enqueue する前に queued Push が FIFO 上で実行されるため、最新 experience は現在 consume する batch には見えず、次以降の prefetched batch から見える。`UpdatePriorities(caller)` は in-flight future の完了を `wait()` してから inner へ進む。この契約は inner mutation が必ず wrapper 経由で行われること、`action_info` を毎 step 新規生成・immutable 扱いすることを前提にする。SDPA や CUDA kernel などの演算 bit determinism は別レイヤで扱う。
...
- **自前パラメータは持たない**：Module 系（accessor / Save / Load / Size / Forward 等）は **inner へ素通し**（checkpoint は inner のもの、変更なし）。`Push` は prefetch 未armed では同期委譲、armed 後は shallow snapshot 付き write-behind で順序を固定する。
...
```
Push:
  if (!prefetch_future_.valid()) inner_->Push(batch);
  else enqueue_fifo(inner_->Push(shallow_snapshot(batch)));  // current future の後、next Fetch の前

UpdatePriorities:
  if (prefetch_future_.valid()) prefetch_future_.wait();
  wait_queued_push_exceptions();
  inner_->UpdatePriorities(indices, priorities);
```
`wait()` は future を消費しない（次 `Sample()` の `get()` が成立）。queued Push は例外伝播用に future を保持し、次の同期境界で `get()` する。
...
- `UpdatePerPriorities`：`MaybeLaunchSamplePrefetch()` を削除し、`replay_buffer_->UpdatePriorities(...)`（PrefetchingReplayBuffer が wait を挟む）だけ残す。`UpdateFromBatch` 冒頭の `Push` も PrefetchingReplayBuffer 経由で、未armed なら同期、armed 後なら write-behind される。
...
4. **SampleIndices は background `Sample()` 内で実行**。caller thread へ戻さず、armed 後 `Push()` の FIFO write-behind と `UpdatePriorities()` の wait 境界で mutation との順序を固定する。

## 3. 外部仕様（config）

`learner.use_rb_prefetch`（bool, default false）。**key は変更なし**。true で `Learner` が `replay_buffer_` を PrefetchingReplayBuffer で wrap し、Sample+To prefetch と armed 後 Push write-behind を有効化する。DefaultDQN/QR は CPU/CUDA とも有効化可能。Rainbow は現状どおり config 側で false 固定。
...
| `core/anet-core/src/replay_buffer_impl.cpp` | `PrefetchingReplayBuffer` 実装、armed 後 `Push` write-behind、`UpdatePriorities` 前 wait、`PinCpuSamples` 等の helper も同じ機能グループ内へ移設 |
...
2. **計測**：`ANET_PROFILE_SCOPE` 系で分割 ―― `PrefetchingReplayBuffer::Push.snapshot` / `PrefetchingReplayBuffer::Push.push_worker` / `PrefetchingReplayBuffer::UpdatePriorities.wait_prefetch` / `PrefetchingReplayBuffer::Sample.consume_wait` / `PrefetchingReplayBuffer::Fetch.sample` / `PrefetchingReplayBuffer::Fetch.to`。`Sample+To` と armed 後 `Push` は worker 側で overlap する。
...
5. **整合性（credit loop 0回 / accessor）**：先読みが呼び出しを跨いで in-flight のまま次 Push が来ても、runner 側 stable 化、shallow snapshot、worker FIFO、inner storage lock で安全。metrics accessor 並行も lock 防御。
6. **ビルド**：VsDevCmd x64-Debug で `anet-core-test`。

## 7. Out of Scope

- **N-deep prefetch（AsyncDataLoader）**：`docs/memo/999_async_loader_ndeep.md` に分離。本 PRD は 1-deep + armed 後 Push write-behind のみ。
- **N-deep への拡張 / MuZero / `indices_cpu` D2H 同期除去 / Push 所有権再設計**。
