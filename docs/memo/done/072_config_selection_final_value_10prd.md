...
**ゴール: `$`でベースを組み立て、その場所の個別指定・部分指定で上書きできる。複数の選択元は各最終値を右側の差分で重ねる。ベースに負ける既定値は`?=`の既定葉として同じ位置に書く。** 名前のドット数、A1等の予約名、`[key]`の有無で継承の規則を変えない。

上書き層の名称・段数は設定運用上の約束として残せるが、resolverが識別する種類にはしない。既定葉を表す`?=`以外の新しい構文、モード、互換スイッチ、キー削除演算子、葉の削除用の由来追跡は追加しない。
...
| 弱い既定値の直書き | 選択で上書きされる | `=`は強い個別指定になる。選択に負ける既定値は`?=`の既定葉として同じ位置に書く |

最終値の伝播、Run・CLIの同一キーに対する優先、未選択プロファイルの休止、未定義・自己供給・循環・深さの検証、`${}`の1段参照は維持する。公開APIとresolution JSONのフィールド構成は変更しない。入力記法に追加するのは`?=`だけである。
...
### P2: 同じ設定では個別・部分指定を優先し、既定葉はベースに負ける

1. その設定の配下に`?=`で書いた既定葉を最下層に置く。`?=`は葉にだけ書ける。全体・部分の`.$`への`?=`(`Env.$ ?= @base`)は読み込み時にfail-fastにし、選択宣言は従来の`=`と後勝ち(P3)だけで扱う。
2. その設定の`.$`で全体のベースを作る。ベースが供給するキーは既定葉に勝つ。
3. その設定の配下に書いた部分の`.$`を重ねる。全体より具体的な部分の指定が強い。
4. その設定に`=`で直接書いた個別葉を重ねる。同じ葉を供給するベース・部分選択より個別葉が強い。

異なるキーの宣言を並べ替えても、この優先関係は変わらない。プロファイル内外で同じ規則を使う。`Env.@a.$ = @b`と`Env.@a.value = own`では、@bにvalueがあってもownが勝つ。通常の`Env.value = own`も`Env.$`の結果に勝つ。プロファイル内に書いた部分の`.$`(`Agent.@base : actor.[eval].policy.$`)はプロファイル自身の名前空間で値を確定し、外側の選択(`Agent.$ = @base`)を通してベースの値として届く。同じ場所へrootで直接書いた部分の`.$`(`Agent.actor.[eval].policy.$`)はそのベースより強く、行順を逆にしても変わらない(受入例「Partial root selection beats profile supplied selection」)。プロファイル内の`?=`も同じ規則であり、そのプロファイル自身の`.$`に負けるだけで、外側の設定へは通常のベース値として届く。

この具体性の比較は、各選択元を組み立てる範囲の規則である。**別々の選択元を`A2 > A3`で重ねるときは、A3の最終値がA2の同名キーに勝つ。** A2内に深い部分指定があっても、後段A3の値より強くしない(M19)。A1等の数字自体には優先順位がない。

`=`で直接書いた値は既定値ではなく個別指定である。選択に負ける既定値は`?=`で同じ位置に書く(§7.1)。既定葉は選択元が同じキーを供給しない場合だけ最終値に残る。既定値専用のプロファイル名や暗黙の補完は設けない。意図した個別葉が継承値に勝つのは正当な指定であり、この競合を理由とするWARNは出さない。外側の選択でA2から供給した値は、参照先に直接書いてある葉や部分指定を無条件には上書きしない(M02、M07)。
...
同じキーに`?=`と`=`が混在した場合は、順序に関係なく`=`を採用する。`?=`同士、`=`同士は後勝ちである。後から書いた既定葉が個別指定を取り消すことはない。この強弱の判定は通常設定の葉の間だけで行い、Runプロファイル・CLIの葉には適用しない(P4)。
...
通常の設定値はP1〜P3で決め、そのキーへのRunプロファイルの明示指定が勝ち、CLIの明示指定がさらに勝つ。解決入力キー(`.$`・`@`を含む定義)でもCLIをRunプロファイルより強くする。Runプロファイル・CLIの葉は通常設定の`=`より強い別の段であり、通常の`?=`・`=`の強弱判定には入らない。Runプロファイル内とCLIの`?=`は受け付けず、fail-fastにする。選択宣言`.$`への`?=`も受け付けず、読み込み時にfail-fastにする。
...
| `E1.ground_y=0.2` | E1の葉。`LunarLanderEnv.$ = @trunk > E1`なら0.2が届くが、別キー`LunarLanderEnv.ground_y=0.5`を個別指定した独立分岐では0.5が勝つ |
| `LunarLanderEnv.$=LunarLanderEnv.@trunk>E1` | 選択キーだけ。file・Runの同じチェーンを丸ごと置き換える。実効葉の個別指定を消したり上書きしたりせず、`?=`の既定葉も残る |

選択キーのCLIやRunでチェーンを置き換えても、`?=`で書いた既定葉は選択と独立に残る。既定値のためにチェーン先頭へ特定のプロファイルを明示する必要はない(§7.1)。
...
`references`は従来の1段値参照を、参照元キー順に並べて記録する(既定値の配置移行で入力順が変わっても診断が安定するため。参照先と値は変えない)。`overrides`はRunの最終指定が同じキーのRun適用前の最終値を変えた場合だけ、`key` / `by` / `from` / `to`で記録する。途中の4→1→4は載せない。`to`はRunの値であり、同じキーをCLIが上書きした後の実効値とは異なりうる。最終値は`config_data.txt`で確認する。
...
4. `?=`は既定葉。その設定のベースが同じキーを供給しなければ残り、供給すれば負ける。葉にだけ書け、`.$`・Runプロファイル・CLIでは書けない。
...
| `AtariEnv.game ?= pong` | 既定葉。`AtariEnv.$`の選択元やE1が`game`を供給すればそちらが勝ち、供給しなければpongが残る |
...
| A03 | M05、M09 | 個別葉はベースに勝ち、既定葉はベースに負け、同じ設定の部分指定は全体に勝つ。行順の逆転でも不変 |
| A04 | M06、M18 | 別々の選択元は差分合成で葉を残す。同じ入力キーの再指定は最後だけを採用し、`?=`と`=`の混在は順序に関係なく`=` |
...
| A12 | M15 | Runの項の後勝ち、CLIの最優先、解決入力キーとrun.$の指定。Run・CLIの`?=`はfail-fast |
| A13 | M01、M16、M14 | 定義位置の記録、run.$先頭、Runの最終差分だけのoverrides、schema_version=1 |
| A14 | 既存入力・値参照テスト | include・同一キー再指定・パーサ、`${}`1段と異常系を維持 |
| A15 | §7の17入力・代替golden・静的検査 | 既定葉へ書き換えた17入力の値一致を必須にし、解決記録の差分を理由付きで確認。17入力に含まれないチェーンは代替goldenとソート比較し、チェーンを持つownerの配下に意図しない`=`直書きが残っていないことを確認 |

既定葉の受入分岐はA02・A03・A04・A12・A15へ追加する。以下は後続実装の検証条件である。

- `LunarLanderEnv.ground_y`等を`?=`にした入力で、E1や選択元から変更できること。Agent側もA2から既定葉を変更できること。同値だった5キーも後段から異なる値に変えて検証する。
- 同じ実効葉を意図して個別指定した独立分岐では、その値が継承値に勝ち、競合WARNを出さない。個別葉と選択行を逆順にしても同じ結果になること。
- 同じキーに`?=`と`=`が混在する入力で、順序に関係なく`=`が勝つこと。プロファイル内の`?=`がそのプロファイルの`.$`に負け、外側へはベース値として届くこと。
- P4の3種類のCLIを分けて検証する。対象の葉へのCLIは必ず勝ち、E1内の葉へのCLIはE1の最終値を変え、選択キーへのCLIはそのチェーンだけを置き換えること。Runプロファイル内とCLIの`?=`、全体・部分の`.$`への`?=`はfail-fastになること。
- file、Run、CLIの各チェーン置換経路で、選択先が持たないキーの既定葉が残ること。backendの`deterministic_warn_only`とappの`log_flush_interval_ms`を含め、プロファイルが全キーを持つという偶然に依存しないこと。
- 旧goldenを保持し、既定葉へ書き換えた17入力の全キー・文字列値を一致させる。解決記録は値のgateから分け、§7.2に従って確認する。
...
| 既定値の例 | 既定値専用プロファイルやチェーン先頭の明示を前提にせず、既定葉`?=`とチェーン不変で書く。Run・CLIの`?=`のfail-fast、`?=`と`=`の混在、プロファイル内の`?=`の例を含める |

## 7. 現用設定の移行と17件の比較

### 7.1 チェーン配下の弱い既定値を既定葉`?=`へ書き換える

新規則は既定値の意味も変える。`=`の直書きが強い個別指定になるため、チェーン配下に置いた弱い既定値は、同じ位置のまま`?=`の既定葉へ書き換える。**書き換え範囲は、チェーンが値を供給するコピー先prefix配下の弱い既定値全体**とする。現在値が異なる24キーだけに限定せず、同値の5キーと未競合として報告された149キーも棚卸し対象に含める。チェーン宣言、Runプロファイル、bat、設定生成ツールのCLIは変更しない。既定葉は選択と独立に残るため、チェーン先頭に既定値用のプロファイルを明示する必要も、共有チェーンのための空プロファイル定義も無い。§7.2の17入力について、固定configの個別葉と旧goldenを照合する。以下はRun・CLIによる同一キーの明示指定を考慮した静的棚卸しであり、新resolverを実行した検証結果ではない。対象24キーの個別葉は、2026-09-13の現用設定とmanifestの固定configで同一だった。
...
```text
# common.txt: 8キーすべてを既定葉にする。値一致に直接影響する3キーの抜粋
backend.cudnn_benchmark ?= false
backend.deterministic_algorithms ?= true
backend.cudnn_deterministic ?= false
```

既定葉は、選択元が同じキーを供給するときだけ負け、供給しないキーではそのまま残る。`backend.$ = backend.@non-deterministic`、`run.@repro : backend.$ = backend.@deterministic`、batの`BK`はそのまま使える。backend.$を指定しないGridMaze / GridMaze_muzero / CartPoleも既定葉の値を得る。backend.@non-deterministic / backend.@deterministicが持たない5キーはどの置換経路でも残り、選択先が全キーを持つことを既定値保持の条件にしない。
...
先行版ではbreakout、`=`のまま新規則を適用すると個別指定が勝ってpongになる。pongを弱い既定値として維持する書き換えは次の1行だけで、チェーンとE1は変えない。

```text
AtariEnv.game ?= pong
```

書き換え後もbreakoutを維持する。AtariのIQN出力先でも、直接書かれた`DefaultDQNAgent.net.body.output.[features] = main_feature`と、選択するiqn_fusionが衝突するので同様に`?=`にする。これらを含め、現在同値または未競合でもコピー先に置いた弱い既定値は既定葉にする。上のgameの例はAtariEnvの13定義を書き換えるうちの1キーの抜粋である。

#### 強弱を誤ると17入力で値が変わる24キー

**A15の文字列値一致で数えると24種。これは`?=`を付け忘れた場合に値が変わる感度キーの一覧であり、書き換えの総数ではない。** レビューで挙がった22種に、数値として同じでも表記が異なる`DropMergeEnv.no_drop_timeout_gameover_penalty`と、派生入力lunarlander-reproの`backend.cudnn_deterministic`を加える。22と24は同じ棚卸しの数え方の差である。DropMergeは挙動に関わる14種と表記差1種の計15キーであり、`no_drop_timeout_*`は下表の2キーへ分けて数える。

「旧golden」は`?=`で保持する値、「`=`のまま」は個別葉が勝った場合の値である。キーの種類数は入力をまたいで重複排除し、env別の件数は各envに属するmanifest入力の和集合とする。

| キー | 旧golden | `=`のまま | 影響する入力 |
|---|---|---|---|
...
レビュー時の現用7envの棚卸しでは、「全体↔部分」の値が競合する箇所は0件だった。この報告は現用入力の書き換え量の見積もりであり、P2の部分指定優先を検証した結果ではない。M09・M19の専用例と17入力の実装後比較は引き続き必要とする。
...
| DropMerge.txt | 42 | 42 |
| GridMaze.txt | 15 | 15 |
| GridMaze_muzero.txt | 15 | 15 |
| ImageCls.txt | 11 | 11 |
| LunarLander.txt | 16 | 16 |
| metrics_image.txt | 83 | 87 |
| metrics_scalar.txt | 228 | 228 |
| nn.txt | 124 | 125 |
| nn_cnx.txt | 164 | 164 |
| **全ファイルの和集合** | **944** | **964** |

種類数は同一キーを重複排除し、定義行数は同一キーの再指定も数える。現用設定と固定commitへの移行はいずれも944種類・964定義行となる。共通includeを入力ごとに重複加算しない。

DropMergeの`?=`はEnvデフォルト42行だけとする。NN部品・配線・出力先・ヘッド構造・Run名などは`=`。AtariはEnvデフォルト13行に加え、ヘッド構造2キーと`output.[features]`をAgentのデフォルト設定にまとめる。前者はNatureDQNでReLUへ、後者はIQNでiqn_fusionへ切り替わるために必要な既定葉である。その他の例外は実効値比較で必要性を確認した場合に限る。

文字列表記・コメント・定義位置・重複順を保ち、演算子だけを変更する。既存17入力の旧golden比較に加え、現用設定の通常入力・各Runプロファイル・Optuna追加設定・配線チェーンについて、変更前後の全キー・文字列値を比較する。既定値専用の名前や暗黙の継承は設けない。

#### チェーン・bat・ツールは変更しない

既定葉は選択と独立に残るため、通常設定・Runプロファイル・batのCLI・設定生成ツール・補助テストのチェーン宣言は変更しない。固定17入力への適用も、同じ用途分類で葉の演算子を変えるだけで、manifest・CLI・注入値は保持する。これ以降に追加されたroot直書きの既定値も、同じ性質なら書き換え対象に含める。

**A15の書き換え漏れ検査**: `rg`または`git grep`で、リポジトリ管理下の現用file・`run.@x :`の定義・設定生成ツールを対象に、チェーンを持つownerの配下にある`=`の直書きを全行抽出する。共通ファイルのベースは`?=`、環境別ファイルはデフォルト設定だけ`?=`であることを検査する。環境別のNN部品・プロファイル・メトリクスもそれ以外は`=`とする。キーの用途分類に照らして演算子を検査し、コメントの有無や文言は検査しない。意図した個別指定(DropMerge_optuna.txtのseed指定等)は理由を記録して除外し、残りが0であることを確認する。ownerとプロファイル名を手で列挙して検索範囲を狭めず、抽出結果にはファイル・行・owner・キーを残す。これは書き換え漏れを検出する静的検査であり、resolverの新しい構文制約や実行時WARNではない。§6の値・CLI・既定葉保持の検証と併用し、Runプロファイルや既定値を追加するたびの手順として設定ガイドにも載せる。

キー削除やgoldenの文字列正規化で差分を隠さず、既存のユーザー変更を保持して現用設定と固定入力へ同じ書き換えを適用する。無関係な並べ替え、互換モード、resolverによる暗黙の既定値補完は追加しない。
...
- **値**: 旧goldenと、対応する既定葉へ書き換えた入力の全キー・値の一致を17件すべてで必須とする。診断用の非@prefixも含め、比較から都合の悪いキーを除外しない。
- **書き換え漏れ**: §7.1の静的検査で、チェーンを持つownerの配下に残る`=`の直書きが意図した個別指定だけであることをA15の必須条件とする。17入力に含まれないRun・起動経路も対象にする。
- **書き換えの重点確認**: §7.1の用途分類と定義行数を照合し、24キーを入力別に確認する。同値5キーを含む後段変更と、Run・CLIでチェーンを置き換えたときの既定葉の保持は§6の独立分岐でも検証する。backendの通常選択・既存batと同じCLI選択・Runのrepro選択、DropMergeの14個の値変化と1個の表記差を含める。24キーだけへの比較縮小は行わず、全キー一致と、変更0件だった3envの値保持も確認する。
- **解決記録**: 定義位置に基づく選択・参照・Run差分を新仕様の期待値と比較する。期待値は`expected.py`で入力宣言と旧goldenから独立に算出し、実行結果から作らない。旧記録との違いは入力ごとに理由を示す。既存の空overrides確認を黙って外さない。
- **順序**: 機械比較はキーでソートしてから行い、行順の差分は比較しない(dumpに重複キーはない)。Runのdumpを人間が読む用途で読みづらくなっていないかは別途確認する。
- **代替golden**: 17入力に含まれないチェーン(固定commitより後に追加されたRunプロファイル等)は、改修前に採取したRunの`config/config_data.txt`を代替goldenとし、同じチェーンの短い配線Runのdumpとソート比較する。採取元は旧リゾルバのRunでも、旧リゾルバとの一致を確認済みのチェーンを実装前のツリーで流したdumpでもよい。採取条件(チェーン・CLI・除外キー)を記録に残し、予算・Run名・意図した変更キーは理由を記録して除外し、残りの全キー一致を確認する。この確認は既定葉の実装後に改めて行う。
- **採取証拠**: 旧goldenのハッシュ・採取条件・元ファイルを保持する。差分を消すために一括再採取しない。既存goldenを人間が削除してから明示captureする運用と、比較器の上書き拒否・manifest整合性は維持する。

レビュー後の実装では、固定入力への書き換え内容と新しい記録の期待値を再実行可能な形で残し、比較器・AGENTS.mdの手順を同じ変更で更新する。現用設定の追加変更を旧goldenへ混ぜず、今回の書き換え差分を区別する。

VsDevCmd経由の通常Debugビルド、設定テスト全体、17入力の比較、capture拒否、旧判定の識別子検索、`git diff --check`、UTF-8 / LFを検証する。

## 8. 検討経緯と簡素化の判断

2026-09-12版は、チェーン差し替えで旧選択の葉を消すために宣言の扱いを分けた。2026-09-13の再確認では「継承は差分適用」「$はベース、個別指定で上書き可能」を採用し、その区別の前提を改めた。既存テストの結果を優先して、意図と逆の仕様を残さない。同日、弱い既定値を`@defaults`プロファイルへ移して各チェーン先頭に明示する形を実装・検証したが、Runプロファイルを1本足す際にPRDレビュー直後でも先頭の`@defaults`が抜け、C++既定値と一致するため気づけなかった。共有チェーンのために空のプロファイル定義も要った。1つの`=`に下書きと上書きの両方の意味は載らないため、既定値の表現を`?=`の既定葉へ改めた。
...
| 既定葉の記法`?=` | keep | 下書きと上書きを別の記法で表す。名前・位置に依存せず、チェーン・bat・ツールを変えない |
| 弱い既定値の`@defaults`全面移行とチェーン先頭の明示 | cut(取り下げ) | 既定値の脱落が実際に起き、C++既定値と一致して気づけない。空プロファイル定義と静的検査が要る。現行設定での利得は0件 |
| 先頭プロファイルが`@defaults`を継承する慣用句 | cut | 使う側の負担は消えるが、完成品と差分の分類規約が要り、中間項に付けると既定値が左側を潰す |
| `@defaults`の暗黙補完 | cut | 予約名が増え、既定値を敷かない選択が書けない |
| rootとプロファイル内で規則を分ける | cut | 名前による分類の再導入。`[key]`の扱いが割れる |
| `=`を下書き、`:=`を上書きとする記法 | cut | プロファイル内の`=`が再びベースに負ける。上書き記号に既知の意味がない |
| 個別葉と継承値の競合WARN | cut | P2が認める意図的な上書きにも警告するため。既定値は`?=`で区別し、正当な個別指定に診断を追加しない |
| 互換モード・削除演算子 | cut | 合意した差分合成に不要 |
| 固定入力・旧golden・差分レポート | keep | 17件の実験条件を変えないことと、診断変更を機械的に確認する |
| 契約と実装設計の同期境界 | keep | PRD／ADRで契約をレビューし、docs/designは対応するコード変更と同時に同期する |

成功は「上書き層を識別するコードが不要」「個別・部分指定の行順で結果が変わらない」「書き換え後17件の値一致」「チェーンを足しても既定値が落ちない」で測る。過去の検証は20implへ参照を残し、本改訂の受入条件の証拠とは区別する。

## 9. 今回扱わないこと

- Actor APIや設定カタログへの移行(PRD 061)、NNブロックの未知キー検証など消費側の契約。
- 上書き層の運用上の命名・段数の再設計、未使用キーの自動削除。
- 既定葉がチェーンに隠された、または個別葉がチェーンの値を隠したことの記録。必要になれば`overrides`と同じ絞りで別途検討する。
- `?=`以外の代入演算子(追記・削除)。
