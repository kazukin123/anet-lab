...
- 不正値、未知の列挙値、範囲外の値、互換性のない組み合わせを検出した場合は、暗黙に既定値へフォールバックしない。
- 設定ミスが疑われる値は、起動時または設定読み込み時に明示的なエラーとして扱い、ユーザーが修正できるようにする。
...
実装内のログ、UI 文言、出力ファイルの文言は、周辺コード、既存仕様、個別指示を優先してください。
