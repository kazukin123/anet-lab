...
SweepHeatMapFrame::SweepHeatMapFrame(wxWindow* parent, const wxString& title,
    const SweepHeatMapSettings& settings, std::shared_ptr<anet::rl::DefaultTrainer> trainer)
    : wxFrame(parent, wxID_ANY, title, wxDefaultPosition, wxSize(600, 600))
{
    // メインのレイアウト
    wxBoxSizer* main_sizer = new wxBoxSizer(wxVERTICAL);
    ...
    step_text_->SetSize(wxSize(100, -1));
    ctrl_sizer->Add(step_text_, 0, wxALIGN_CENTER_VERTICAL | wxALL, 5);
    ...
    // --- イベントバインド ---
    timer_.Bind(wxEVT_TIMER, &SweepHeatMapFrame::OnTimer, this);
    refresh_combo_->Bind(wxEVT_COMBOBOX, &SweepHeatMapFrame::OnComboSelect, this);
    canvas_->Bind(wxEVT_PAINT, &SweepHeatMapFrame::OnPaint, this);
    canvas_->Bind(wxEVT_SIZE, &SweepHeatMapFrame::OnSize, this);
    refresh_button->Bind(wxEVT_BUTTON, &SweepHeatMapFrame::OnRefreshButton, this);
...
void SweepHeatMapFrame::CreateObserver(const SweepHeatMapSettings& settings,
...
SweepHeatMapFrame::~SweepHeatMapFrame()
{
    // タイマー停止
    if (timer_.IsRunning()) {
        timer_.Stop();
...
void SweepHeatMapFrame::OnComboSelect(wxCommandEvent& event)
{
    UpdateRefreshRate();
}

void SweepHeatMapFrame::UpdateRefreshRate()
{
...
void SweepHeatMapFrame::OnPaint(wxPaintEvent& event)
{
...
void SweepHeatMapFrame::OnSize(wxSizeEvent& event)
{
