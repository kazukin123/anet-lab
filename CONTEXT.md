...
**選択チェーン**:
`コピー先.$ = 選択元 > 差分`の形で、各選択元の最終値を右側の差分で重ね、その設定のベースを作る指定。ベースに対する個別・部分指定と、同じ選択キーの再指定は区別する(PRD 072改訂契約)。既定葉はチェーンの置き換えと独立に残る。
_Avoid_: include(入力を取り込む処理), マージ(適用規則が曖昧)
...
_Avoid_: 完成結果(旧称。カタログ限定の含み), スナップショット(旧挙動)

**Run プロファイル**:
Runを特徴づける環境・Agent・予算等の選択と、宣言済み値スロットの値の組を`run.@<name>`で命名したもの(`run.seed`は含めず、同一プロファイルと複数seedを比較母集団とする)。通常設定を上書きするRunの指定であり、同じキーへのCLI指定がさらに優先する。
_Avoid_: 幹(旧称), Run設定(別概念), プリセット

**上書き層**:
チェーンの後段へ置く、ファイルローカルの差分キー群。A1/A2/A3・E1・M1/M2・P1は対象(A=Agent、E=Env、M=Metrics、P=app)と恒久度(大番号ほど一時的)を示す運用上の名前であり、設定継承では通常prefixとして扱う。
_Avoid_: 実験レイヤ(曖昧), patch, 旧名 R/X/O(A2/A3/M2へ吸収済み)

**個別指定**:
ベースから継承した値に対して、その設定の葉キーへ`=`で直接与える値(個別葉)。プロファイル内外ともベースより優先し、同じキーへのRunプロファイル・CLIの指定にだけ負ける。
_Avoid_: デフォルト直書き(旧契約では選択に負ける指定を指した), 上書き(上書き層と混同)

**既定葉**:
`?=`で書く既定値。その設定の`.$`が同じキーを供給しない場合だけ最終値に残り、選択チェーンの置き換えと独立に存在する。同じキーに`=`があれば順序に関係なく`=`が勝つ。葉にだけ書け、選択宣言(`.$`)・Runプロファイル・CLIでは書けない。
_Avoid_: 弱い葉(上書き層A1〜A3の強弱と混同), 下書き(口語), @defaults(取り下げた既定値専用プロファイル)
...
**排水**（drain）:
終了経路で、進行中の背景評価セッションを完走またはキャンセルで終わらせてから、ログとメトリクスの記録先を閉じる手順。完走したセッションは通常どおり記録し、キャンセルしたセッションは何も集計しない。
_Avoid_: flush（出力バッファの書き出し。排水の後段の別工程）, 完走待ち（キャンセルと記録先を閉じる順序を含まない）, 部分集計（キャンセルは集計しない）
...
**Run色**:
Metrics ViewerがRunへ割り当てる表示色で、graphの線・bandとRun listのchipが同じ色を共有する。今の比較の中での役割であってRunの属性ではなく、保存しない（reloadとworkspace切替で配り直す）。見分けやすさを保証するのは選択中Runの集合に対してだけで、未選択Runとの重複は許す。Runの同一性を担うのはrunIdであり、色ではない。
_Avoid_: palette（色の候補集合であって割り当てではない）, 凡例色（凡例はrunIdを出す別要素）, Run識別色（同一性を色が持つと読める）

**分離距離**:
2つのRun色の見分けやすさを測るOKLab空間のユークリッド距離。sRGBの数値差やHSLの角度差は同じ量でも見え方が揃わないため使わない。現行の20色paletteは0.04程度の近接ペアを含み、選択本数が増えるほど達成できる最小距離は下がる（5本で0.16、7本で0.12を下回る）。
_Avoid_: 色差（CIEのΔE*abと混同）, コントラスト（背景との明度差を指す別概念）
