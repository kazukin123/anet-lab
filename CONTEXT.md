...
**選択チェーン**:
`コピー先.$ = 選択元 > 差分`の形で、各選択元の最終値を右側の差分で重ね、その設定のベースを作る指定。ベースに対する個別・部分指定と、同じ選択キーの再指定は区別する(PRD 072改訂契約)。
...
**個別指定**:
ベースから継承した値に対して、その設定の葉キーへ直接与える値。プロファイル内外ともベースより優先する。
_Avoid_: デフォルト直書き(旧契約では選択に負ける指定を指した)
