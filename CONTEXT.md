...
**選択チェーン**:
`コピー先 : $ = 選択元 > 差分` の形で、プロファイル・カタログ・上書き層を左から右へ合成する指定。カタログ項目では、その項目または配下を組み立てる継承と差分の定義にもなる。
...
**カタログの完成結果**:
カタログ項目または参照する部分への設定指定を反映し終えた、値と存在するキーの集合。利用側の型付き Config による既定値補完とは別の、設定解決上の結果を指す。
